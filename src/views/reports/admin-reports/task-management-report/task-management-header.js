...
function TaskManagementHeader({ slug, onChangeHandler, customers }) {
  ...
  const filterOptions = ['Select', 'Corporate', 'Industrial']
    ...
    <ReportsWrapper className='report-wrapper' bordered ref={headerRef}>
      <Box sx={{ flexGrow: 1, marginBottom: { xs: !tableData && "4rem", md: '0'} }}>
        <Grid container rowSpacing={5} columnSpacing={6} xs={{ alignItems: 'center' }}>
          <Grid item xs={12} md={12} xl={9.5} flexWrap='wrap'>
            <Grid container columnSpacing={5} rowSpacing={4}>
                  ...
                  <Grid item xs={12} sm={9}>
                    ...
                    >
                      {assetOptions?.map((asset, index) =>
                        index === 0 ? (
                          <MenuItem key={index} value=''>
                            <PlaceholderText>Select Technician</PlaceholderText>
                          </MenuItem>
                        ) : (
              ...
              <Grid item xs={12} sm={6} md={3}>
                <Grid container alignItems='center' rowSpacing={1}>
                  ...
                  <Grid item xs={10} sm={6.5} md={7}>
                    ...
                    >
                      {filterOptions.map((name, index) =>
                        index === 0 ? (
                          <MenuItem key={name} value=''>
                            <PlaceholderText> Job Type </PlaceholderText>
                          </MenuItem>
                        ) : (
                          <MenuItem key={name} value={name}>
                            {name}
                  ...
                  <Grid item xs={1} md={3} pl={3} display="flex" justifyContent={{ xs: 'end', md: 'start'}} ml={{xs: 'auto', md:'0'}}>
                    <FormControlLabel
                      sx={{marginRight: { xs: 0, sm: 4}}}
                      control={<Checkbox defaultValue={false} />}
                      label={<Typography sx={{ fontWeight: '600', textAlign: 'center' }}>All</Typography>}
                    />
                  </Grid>
