...
function MileageReportHeader({ slug, onChangeHandler, customers, toggleTableData, getTableHeight }) {
  const common = useCommonStyles()
  const customStyles = useCustomStyles()
  const datepickerStyles = useDatepickerStyles()
  const { tableData } = useContext(TableUIContext)
        ...
        <Grid container rowSpacing={5} columnSpacing={6} xs={{ alignItems: 'center' }}>
          <Grid item xs={12} md={12} xl={9} flexWrap='wrap'>
            <Grid container columnSpacing={5} rowSpacing={4}>
              ...
              <Grid item xs={12} sm={6} md={3}>
                <Grid container alignItems='center' mb={3} rowSpacing={1}>
                  <Grid item xs={12} sm={2.5} md={5.5}>
...
export default MileageReportHeader

MileageReportHeader.propTypes = {
