...
function FleetSummaryReportHeader({ onChangeHandler }) {
  // ** Router
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      ...
      if (!values.from_date && isObjEmpty(formik.errors)) {
        ...
        dispatch(getAllFleetSummaryActions({ page: 1, limit: 10, asset_id: values?.asset_id }))
  ...
  const clearForm = () => {
    ...
    dispatch(getAllFleetSummaryActions({ page: 1, limit: 10 }))
  }
              ...
              <Grid container rowGap={4} columnGap={3}>
                ...
                <Grid item xs={12} sm={6} md={3.5}>
                  <Grid
                    container
                    alignItems='center'
                    sx={{ justifyContent: { sm: 'center', md: 'end' } }}
                    rowSpacing={1}
                    mb={3}
                  >
                    <HeaderDatePicker
...
export default FleetSummaryReportHeader
