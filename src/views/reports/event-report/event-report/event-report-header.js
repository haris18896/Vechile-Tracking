...
function EventReportHeader({ onChangeHandler }) {
  // ** Router
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      ...
      if (!values.from_date && isObjEmpty(formik.errors)) {
        ...
        dispatch(getAllEventReportAction({ page: 1, limit: 10, asset_id: values?.asset_id }))
  ...
  const clearForm = () => {
    ...
    dispatch(getAllEventReportAction({ page: 1, limit: 10 }))
  }
                  ...
                  <Grid container rowSpacing={3} alignItems='center'>
                    ...
                    <Grid item xs={12} sm={12} mt='1.2rem'>
                      <Autocomplete
                        ...
                        // getOptionLabel={option => option.label}
                        // onChange={(e, value) => {
                        //   formik.setFieldValue('event_id', value?.value || '')
                        // }}
                        // sx={{
                        //   '& .MuiOutlinedInput-notchedOutline': {
                        //     borderColor: formik.touched.event_id && formik.errors.event_id && '#E53E3E !important'
                        //   }
                        // }}
                        // value={
                        //   !!formik.values?.event_id
                        //     ? assetList?.find(customer => customer.value === parseInt(formik.values.event_id))
                        //     : null
                        // }
                        renderInput={params => (
                          <TextField
                            {...params}
                            variant='outlined'
                            placeholder={t('reports.placeholder.eventAutoComplete')}
                          />
                        )}
                        className={styles.AutoCompleteSelect}
                      />
