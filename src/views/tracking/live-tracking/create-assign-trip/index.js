...
function CreateAssignTrip() {
  ...
  const filteredRows = rows?.filter(row => row.profile_type?.id === 1)
  ...
  return (
    <>
      <TripHeader
        ...
        create={ability.can('create', 'create-teacher')}
        add={() => router.push('create-assign-trip/add')}
      />
      ...
      <TripTable
        ...
        total={total}
        router={router}
        update={ability.can('update', 'update-teacher')}
        loading={loading}
        rows={filteredRows}
...
CreateAssignTrip.acl = {
  action: 'manage',
  subject: 'manage-teacher-profile'
}

export default CreateAssignTrip
