...
function NearestAsset(props) {
  ...
  const schema = Yup.object().shape({
    ...
    search_by: Yup.string().required('Search parameter is required'),
    asset_name: Yup.string().required('Asset Name is required'),
    geofence: Yup.string().required('Geofence is required'),
          ...
          >
            ...
            >
              ...
              <Grid container mt="64px">
                <Grid item xs={12}>
                  <HeaderLabel sx={{ color: '#556485', fontSize: '1rem'}}>Search Nearest Vehicle</HeaderLabel>
                </Grid>
                <NearestAssetForm formik={formik}  />
              </Grid>
              </TrackingWrapper>
            </Grid>
          </SwipeableDrawer>
