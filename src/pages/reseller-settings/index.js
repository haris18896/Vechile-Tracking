...
function ResellerRoles() {
  ...
  // ** States && constants
  const updateAbility = 'update-reseller-roles'
  const deleteAbility = 'delete-reseller-roles'
  const [dialogType, setDialogType] = useState(null)
  const [selectedRow, setSelectedRow] = useState(null)

  // ** functions
  const handleUpdate = row => router.push(`/reseller-settings/roles/add-edit/${row?.id}?edit=${true}`)
  ...
  return (
    <>
      <SettingsHeader
        ability={ability}
        addBtnText={t('Add')}
        abilityType={'create-reseller-roles'}
        title={t('settings.roles.rolesList')}
        addClick={() => router.push(`/reseller-settings/roles/add-edit/${0}`)}
      />
