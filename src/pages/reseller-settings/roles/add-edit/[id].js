...
function AddEditResellerRoles({ props }) {
  // ** Translation
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setSubmitting, resetForm }) => {
      if (isObjEmpty(formik.errors)) {
        if (edit) {
          dispatch(updateRoleAction({ data: values, id, callback: () => router.push('/reseller-settings/roles') }))
        } else {
          dispatch(registerRoleAction({ data: values, callback: () => router.push('/reseller-settings/roles') }))
        }
      }
    }
  })
  ...
  return (
    <SettingsWrapper>
      <SettingsHeaderAddEdit
        ...
        backAction={() => router.push('/reseller-settings')}
        title={edit ? `${t('edit')} ${t('settings.roles.roles')}` : `${t('add')} ${t('settings.roles.roles')}`}
      />

      <form name='roles-form-reseller' onSubmit={formik.handleSubmit}>
...
export default AddEditResellerRoles

AddEditResellerRoles.acl = {
  action: 'manage',
