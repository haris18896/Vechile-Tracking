...
function ResellerDeviceTypes() {
  ...
  // ** States && constants
  const updateAbility = 'update-reseller-devices'
  const deleteAbility = 'delete-reseller-devices'
  ...
  // ** functions
  const handleUpdate = row => router.push(`/reseller-settings/devices/add-edit/${row?.id}?edit=${true}`)
  ...
  return (
    <>
      <SettingsHeader
        ability={ability}
        addBtnText={t('Add')}
        abilityType={'create-reseller-devices'}
        title={t('settings.deviceTypes.deviceTypesList')}
        addClick={() => router.push(`/reseller-settings/devices/add-edit/${0}`)}
      />
...
export default ResellerDeviceTypes

ResellerDeviceTypes.acl = {
  action: 'manage',
