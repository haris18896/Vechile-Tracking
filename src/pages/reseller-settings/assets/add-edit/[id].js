...
function AddEditResellerAssetTypes() {
  // ** Translation
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      if (isObjEmpty(formik.errors)) {
        if (edit) {
          dispatch(
            updateAssetTypeAction({ data: values, id, callback: () => router.push('/reseller-settings/asset-types') })
          )
        } else {
          dispatch(
            registerAssetTypeAction({ data: values, callback: () => router.push('/reseller-settings/asset-types') })
          )
        }
      }
    }
  ...
  return (
    <SettingsWrapper>
      <SettingsHeaderAddEdit
        ...
        back={t('back')}
        loading={loading}
        backAction={() => router.push('/reseller-settings/asset-types')}
...
export default AddEditResellerAssetTypes

AddEditResellerAssetTypes.acl = {
  action: 'manage',
