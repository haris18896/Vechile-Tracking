...
function CustomerAssetTypes() {
  ...
  // ** States && constants
  const updateAbility = 'update-customer-assets'
  const deleteAbility = 'delete-customer-assets'
  ...
  // ** functions
  const handleUpdate = row => router.push(`/customer-settings/assets/add-edit/${row?.id}?edit=${true}`)
  ...
  return (
    <>
      <SettingsHeader
        ability={ability}
        addBtnText={t('Add')}
        abilityType={'create-customer-assets'}
        title={t('settings.assetTypes.assetTypesList')}
        addClick={() => router.push(`/customer-settings/assets/add-edit/${0}`)}
      />
...
export default CustomerAssetTypes

CustomerAssetTypes.acl = {
  action: 'manage',
