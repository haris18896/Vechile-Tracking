...
function CustomerSettings() {
  ...
  // ** States && constants
  const updateAbility = 'update-customer-roles'
  const deleteAbility = 'delete-customer-roles'
  const [dialogType, setDialogType] = useState(null)
  const [selectedRow, setSelectedRow] = useState(null)

  // ** functions
  const handleUpdate = row => router.push(`/customer-settings/roles/add-edit/${row?.id}?edit=${true}`)
  ...
  return (
    <>
      <SettingsHeader
        ability={ability}
        addBtnText={t('Add')}
        abilityType={'create-customer-roles'}
        title={t('settings.roles.rolesList')}
        addClick={() => router.push(`/customer-settings/roles/add-edit/${0}`)}
      />
