...
function AddEditCustomerRoles({ props }) {
  // ** Translation
  ...
  const permissions = getAllPermissionsList?.data
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setSubmitting, resetForm }) => {
      if (isObjEmpty(formik.errors)) {
        ...
        if (edit) {
          dispatch(updateRoleAction({ data, id, callback: () => router.push('/customer-settings/roles') }))
        } else {
          dispatch(registerRoleAction({ data, callback: () => router.push('/customer-settings/roles') }))
        }
      }
    }
  })
  ...
  useEffect(() => {
    const asyncCall = async () => {
      await dispatch(getAllPermissionsAction({ page: 1, limit: 500 }))
      if (id && edit) {
  ...
  return (
    <SettingsWrapper>
      <SettingsHeaderAddEdit
        ...
        backAction={() => router.push('/customer-settings')}
        title={edit ? `${t('edit')} ${t('settings.roles.roles')}` : `${t('add')} ${t('settings.roles.roles')}`}
      />

      <form name='roles-form-customer' onSubmit={formik.handleSubmit}>
...
export default AddEditCustomerRoles

AddEditCustomerRoles.acl = {
  action: 'manage',
