...
function EditWarehouse() {
  ...
  const editWarehouseFormik = useFormik({
    initialValues: {
      city: '',
      name: '',
      phone: '',
    ...
    onSubmit: (values, { setSubmitting, resetForm }) => {
      if (isObjEmpty(editWarehouseFormik.errors)) {
        ...
        // dispatch(updateWarehouseAction({ data, router }))
      }
    }
  })
  ...
  return (
    <>
      <AddEditWarehouseHeader
        loading={false}
        router={router}
        formik={editWarehouseFormik}
        submitHandler={editWarehouseFormik.handleSubmit}
      />

      <AddEditWarehouseForm router={router} formik={editWarehouseFormik} />
    </>
  )
}
