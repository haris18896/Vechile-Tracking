...
import { getAllResellerAction } from 'src/store/settings/reseller/resellerAction'
import { useCustomStyles } from 'src/styles/pages/catalogs'

function AddEditAdminCustomer() {
  // ** Settings
  ...
  const { getAllResellerList } = useSelector(state => state.reseller)

  const resellersList = getAllResellerList?.data

  // ** OnChange Handler for getting Id of Reseller
  ...
  const schema = Yup.object().shape({
    ...
    //   })
    //   .nullable(),
    // reseller_id: Yup.string().required(`${t('settings.customers.addEdit.reseller_id')}`),
  ...
  const formik = useFormik({
    initialValues: {
      ...
      reseller_id: getCustomer?.data?.reseller_id || '',
    ...
    onSubmit: values => {
      if (isObjEmpty(formik.errors)) {
        ...
        data.append('reseller_id', values.reseller_id)
        ...
        if (edit) {
          dispatch(updateCustomerAction({ id, data, callback: () => router.push('/admin-settings/customers') }))
        } else {
          console.log('helo')
          dispatch(registerCustomerAction({ data, callback: () => router.push('/admin-settings/customers') }))
        }
      }
    }
  })
  ...
  // ** Getting Resellers Information
  useEffect(() => {
    dispatch(getAllResellerAction({ page: 1, limit: 'all' }))
  }, [])
  ...
  return (
    <SettingsWrapper>
      <SettingsHeaderAddEdit
        ...
        backAction={() => router.push('/admin-settings/customers')}
        title={
          edit
          ...
          <Grid item xs={12}>
            ...
            <Grid container>
              ...
              <Grid item xs={12} md={6}>
                <Grid item xs={12} sm={10} md={8}>
                  <FieldWrapper>
                    <TextLabel>{t('settings.customers.addEdit.reseller_label')}</TextLabel>
                    <Autocomplete
                      id='reseller_id'
                      name='reseller_id'
                      options={
                      ...
                      onChange={(e, value) => {
                        setResellerId(value?.id)
                        formik.setFieldValue('reseller_id', value?.id)
                      }}
                      value={resellersList?.find(reseller => reseller.id === resellerId)}
                      renderInput={params => (
                        <TextField
                          {...params}
                          id='reseller_id'
                          name='reseller_id'
                          variant='outlined'
                          placeholder={t('settings.customers.addEdit.reseller_placeholder')}
                        />
                      )}
                      className={common.AutoCompleteSelectForm}
                    />
                  </FieldWrapper>
                </Grid>
...
export default AddEditAdminCustomer

AddEditAdminCustomer.acl = {
  action: 'manage',
