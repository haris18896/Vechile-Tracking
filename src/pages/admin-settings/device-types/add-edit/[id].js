...
function AddEditAdminDeviceTypes() {
  // ** Translation
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      if (isObjEmpty(formik.errors)) {
        if (edit) {
          dispatch(
            updateDeviceTypeAction({ data: values, id, callback: () => router.push('/admin-settings/device-types') })
          )
        } else {
          dispatch(
            registerDeviceTypeAction({ data: values, callback: () => router.push('/admin-settings/device-types') })
          )
        }
      }
    }
  ...
  return (
    <SettingsWrapper>
      <SettingsHeaderAddEdit
        ...
        backAction={() => router.push('/admin-settings/device-types')}
        title={
          edit
...
export default AddEditAdminDeviceTypes

AddEditAdminDeviceTypes.acl = {
  action: 'manage',
