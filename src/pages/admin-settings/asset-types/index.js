...
function AssetTypes() {
  ...
  // ** States && constants
  const updateAbility = 'update-admin-asset-types'
  const deleteAbility = 'delete-admin-asset-types'
  ...
  // ** functions
  const handleUpdate = row => router.push(`/admin-settings/asset-types/add-edit/${row?.id}?edit=${true}`)
  ...
  return (
    <>
      <SettingsHeader
        ability={ability}
        addBtnText={t('Add')}
        abilityType={'create-admin-asset-types'}
        title={t('settings.assetTypes.assetTypesList')}
        addClick={() => router.push(`/admin-settings/asset-types/add-edit/${0}`)}
      />
