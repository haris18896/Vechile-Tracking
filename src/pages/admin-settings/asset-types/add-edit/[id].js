...
function AddEditAdminAssetTypes() {
  // ** Translation
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      if (isObjEmpty(formik.errors)) {
        if (edit) {
          dispatch(
            updateAssetTypeAction({ data: values, id, callback: () => router.push('/admin-settings/asset-types') })
          )
        } else {
          dispatch(
            registerAssetTypeAction({ data: values, callback: () => router.push('/admin-settings/asset-types') })
          )
        }
      }
    }
  ...
  return (
    <SettingsWrapper>
      <SettingsHeaderAddEdit
        ...
        back={t('back')}
        loading={loading}
        backAction={() => router.push('/admin-settings/asset-types')}
...
export default AddEditAdminAssetTypes

AddEditAdminAssetTypes.acl = {
  action: 'manage',
  subject: 'manage-super-admin-asset-types'
}
