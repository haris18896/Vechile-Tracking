...
import FuelConsumedAdd from './add'

function FuelConsumedEdit() {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      asset: filteredData?.asset_name || '',
      date: moment(filteredData?.date).toDate() || '',
      time: filteredData?.time || '',
      driver: filteredData?.driver_name || '',
      fuel_filled: filteredData?.fuel_filled || '',
      odometer: filteredData?.odometer || ''
    },
  ...
  return (
    <ServiceWrapper>
      <FuelConsumedAddHeader formik={formik} />

      <FuelConsumed formik={formik} data={data} filteredData={filteredData} />
    </ServiceWrapper>
  )
}
...
FuelConsumedEdit.acl = {
  action: 'manage',
  subject: 'manage-fuel-consumed'
}

FuelConsumedEdit.AuthGaurd = true

export default FuelConsumedEdit
