...
function RulesAdmin() {
  ...
  const rulesData = data?.find(data => data.id === id)
  ...
  const formik = useFormik({
    initialValues: {
      account: rulesData?.account || '',
      email_subject: rulesData?.email_subject || '',
      alert_name: '',
      notification_email: '',
      email_message: '',
      notification_mobile: '',
      system_rule: '',
      sms_message: '',
      active: rulesData?.active || '',
      cron_rule: rulesData?.cron_rule || '',
      asset_id: '',
      all_assets: false,
  ...
  return (
    <ServiceWrapper>
      ...
      <RulesAdminForm formik={formik} customers={data} />
    </ServiceWrapper>
  )
}
