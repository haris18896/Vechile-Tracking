...
import AllocateAssetsAdd from './add'

function AllocateAssetsEdit() {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      assignment_type: filteredData?.assignment_type || '',
      allocate_to: filteredData?.allocation || '',
      mobile_no: filteredData?.mobile_no || '',
      asset_name: filteredData?.asset_name || '',
      emp_id: filteredData?.emp_id || '',
      driver_name: filteredData?.driver_name || '',
      current_odometer: filteredData?.current_odometer || '',
      license_no: filteredData?.license_no || '',
      vehicle_condition: filteredData?.vehicle_condition || ''
    },
  ...
  return (
    <ServiceWrapper>
      ...
      <AssetTypesFrom formik={formik} data={rows} filteredData={filteredData} />
    </ServiceWrapper>
  )
}
...
AllocateAssetsEdit.acl = {
  action: 'manage',
  subject: 'manage-allocate-asset'
}

AllocateAssetsEdit.AuthGuard = true

export default AllocateAssetsEdit
